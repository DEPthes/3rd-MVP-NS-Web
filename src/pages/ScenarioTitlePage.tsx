	...
	const filteredTopics = topics
		...
		.sort((a, b) => {
			switch (sortType) {
				...
				case '최근 관심순 |':
					return (likeDates[b.title]?.getTime() || 0) - (likeDates[a.title]?.getTime() || 0);
				default:
					return 0;
			}
		});
			...
			{filteredTopics.map((topic) => (
				<S.TopicBox key={topic.id} onClick={() => handleTopicClick(topic.id)}>
					<div>
						...
						<S.TopicDate>발행일: {new Date(topic.publishDate).toLocaleDateString()}</S.TopicDate>
					...
					>
						{isLikedTopics[topic.title] ? (
							<img src={BlueHeartFill} alt="Liked" />
						) : (
							<img src={BlueHeart} alt="Not Liked" />
						)}
